...
export default class SideNavBar extends Component<PropsType, StateType> {
    ...
    */
    ...
    state = {
        currentSection: this.props.product.page.sections[0].slug,
        sticky: false,
        bottom: false,
    };
    ...
    handleScroll(e: Event) {
        ...
        const sections = this.props.product.page.sections;
        for (var j = 0; j < sections.length; j++) {
            var elemId = sections[j].slug;
    ...
    renderSVG() {
        ...
        var content = [
            <line key={"nav-line"} x1={20} x2={20} y1={20} y2={20 + 40*(sections.length-1)} style={styles.lineStyle} />
        ];
        for (var i = 0; i < sections.length; i++) {
            var curOffset = i * 40;
            var style = {...styles.diamondStyle};
            var s = 4;
            if (sections[i].slug === this.state.currentSection) {
                s = 10;
                style.fill = "#BA3737";
            }
            var points = [
        ...
        return <svg width={40} height={sections.length * 40}>{content}</svg>;
    }
    ...
    renderSections() {
        ...
        for (var i = 0; i < sections.length; i++) {
            var isSelected = (sections[i].slug) === currentSection;
            var padding = isSelected ? 15 : 5;
            var section = (
                <div key={"nav-section-"+i} style={{...styles.sectionFragment, paddingLeft: padding}}>
                    <HashLink
                        className={this.state.currentSection === sections[i].slug ? "anchor-links selected-anchor" : "anchor-links"}
                        id={ sections[i].slug + "-hashlink" }
                        to={ product.link + "#" +  sections[i].slug }>
                        {sections[i].name}
                    </HashLink>
                </div>
            );
            sectionsViews.push(section);
        }
